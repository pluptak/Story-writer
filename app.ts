...
export async function startChapterRun(dir: string, chapter = 1, cli: CliConfig,
  ...
  try {
    // The run resolves against the author's own catalogs, so a skill they promoted
    // and an origin they configured mean in a scene what they mean in the editor.
    const sc = await loadStory(dir, LIVE.modelOverride ?? undefined, await persistedCatalogs());
