...
const newId = () => `sty_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
...
function editorHtml() {
  ...
  if (!d) return `<div class="lib-inspector-empty"><span>Select a style to edit</span><small>Or create a new reusable writing approach.</small></div>`;
  const c = s.selected;
  return `<div class="lib-editor-head"><div><strong>${esc(d.name || "Untitled style")}</strong><small>ID: ${esc(d.id || "not saved")}</small></div><button class="lib-close" id="stylib-close" aria-label="Close style editor">×</button></div>
  <div class="lib-actions">${button({label:"Duplicate", id:"stylib-duplicate", extraClass:"small"})}${button({label:c?.hidden ? "Restore" : "Hide", id:"stylib-toggle-hidden", extraClass:"small", title:"Hiding entries isn't available yet"})}<span class="lib-soon">soon</span>${button({label:"Delete", id:"stylib-delete", variant:"danger", extraClass:"small"})}</div>
  <div class="lib-tabs"><button class="lib-tab active">Overview</button><button class="lib-tab" id="stylib-ai-tab">AI Assistant <b>soon</b></button><button class="lib-tab" disabled>Revisions</button></div>
  ${s.error ? errorLine(esc(s.error)) : ""}${s.dirty ? warnLine("unsaved changes") : ""}
  <div class="lib-section"><h3>Style identity</h3><p>Define a reusable writing approach. Keep story-specific instructions outside the library.</p>
  ...
  <div class="lib-editor-footer">${button({label:"Cancel", id:"stylib-cancel", disabled:!s.dirty})}${button({label:"Save changes", id:"stylib-save", variant:"primary", disabled:!needsSave(s)})}</div>
...
function createNew() { const s = APP.styleLibrary, x = { ...entryOf({...emptyDraft(), id:newId(), name:"New style"}), hidden:false, version:0, updatedAt:Date.now() }; s.entries.unshift(x); setSelected(x); }
function duplicate() { const s = APP.styleLibrary; if (!s.selected) return; const x = { ...clone(s.selected), id:newId(), name:`${s.selected.name} Copy`, version:0, updatedAt:Date.now(), hidden:false }; s.entries.splice(s.entries.indexOf(s.selected) + 1, 0, x); setSelected(x); }
