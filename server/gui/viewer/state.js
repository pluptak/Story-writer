...
// (`.field = x`, `.add()`/`.delete()`) don't need it: a plain `export const` already lets any
// importer reach in and change them.
//
...
export const APP = {
  ...
  chapter: null,                // {dir, n, text}: a written chapter opened inline on the story page.
                                 // Carries its own dir because reaching the shelf by the tab does
  ...
  agents: null,                 // {dir, id, logs[]}: /runs/llm for the run being read. Carries its
                                 // own dir+id like `chapter` -- one run's agents must never render
                                 // under another's
  ...
  editLoading: false,          // a /story/edit fetch is in flight. The editor starts its own load
                                // from its wiring, which runs on every render -- without this flag,
                                // the render that load schedules starts another, forever.
  ...
  awaitingReader: false,        // that consult is still unanswered -- the run is blocked on a human, not
                                 ...
                                 // Hand-tracked, not derived: render() runs on every SSE frame whatever
 ...
 *  the story and handoff pages explain themselves this way rather than round-tripping. Empty string
 *  when nothing is in the way (the loading window after a pick counts). */
