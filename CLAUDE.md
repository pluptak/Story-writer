...
| [run-and-save.ts](run-and-save.ts) | everything one chapter run does around the scene loop: the out/ directory and its logs, incremental scene.md, retained-run rotation, the chapter snapshot and the catalogs snapshot beside it, and the unfired-beat sidecar the handoff reads |
