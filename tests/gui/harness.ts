...
export type FixtureStory = {
  ...
  scenes: { place: string; question: string; pov: string; length: number; roster: string[]; reach?: Record<string, string[]>; presence?: Record<string, string>; constraint?: Record<string, string[]> }[];
...
export function cardFromStory(dir: string, raw: FixtureStory, name = dir): StoryCard {
  return {
    ...
    scenes: raw.scenes.map(s => ({
      ...
      roster: s.roster, reach: s.reach ?? {}, presence: s.presence ?? {}, constraint: s.constraint ?? {},
    })),
